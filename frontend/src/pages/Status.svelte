...
{:else if $q.data}
  ...
  <p class="muted">
    {status.engine} v{status.version} \xb7 schema {status.schema_version} \xb7
    {status.read_only ? "node read-only" : "node mutation capable"}
  </p>
  <div class="grid">
    <div class="card" role="region" aria-label="System health">
      <div class="kv"><span>Overall</span><span class="tag {healthClass(status.overall)}" role="status">{status.overall}</span></div>
      <div class="kv"><span>Database integrity</span><span class="tag {healthClass(status.integrity)}" role="status">{status.integrity}</span></div>
